...
TEST(TestEcho, echo_get_attributes_and_arguments_for_task)
{
	buffer task_arguments;
	SET_NULL_TO_BUFFER(task_arguments);
	/**/
	...
	uint8_t task_attributes_count = 0;
	/**/
	uint8_t returned = echo_get_attributes_and_arguments_for_task(
						   &task_attributes, &task_attributes_lengths, &task_attributes_count, &task_arguments);
	/**/
	ASSERT_TRUE(returned) << buffer_free_with_inner_buffers(&task_arguments);
	ASSERT_NE(nullptr, task_attributes) << buffer_free_with_inner_buffers(&task_arguments);
	...
	{
#if 0
		if (6 == task_attributes_count)
		{
			ASSERT_TRUE(bool_parse(buffer_data(argument, 0), buffer_size(argument),
								   &returned)) << buffer_free_with_inner_buffers(&task_arguments);
			ASSERT_TRUE(returned) << buffer_free_with_inner_buffers(&task_arguments);
		}
		else if (3 == task_attributes_count || 5 == task_attributes_count)
		{
			ASSERT_FALSE(buffer_size(argument)) << buffer_free_with_inner_buffers(&task_arguments);
		}
		else if (2 == task_attributes_count)
		{
			ASSERT_STREQ("UTF8", buffer_to_string(argument).c_str()) << buffer_free_with_inner_buffers(&task_arguments);
		}
		else if (4 == task_attributes_count)
		{
			ASSERT_STREQ("Info", buffer_to_string(argument).c_str()) << buffer_free_with_inner_buffers(&task_arguments);
		}
		else
		{
			ASSERT_TRUE(bool_parse(buffer_data(argument, 0), buffer_size(argument),
								   &returned)) << buffer_free_with_inner_buffers(&task_arguments);
			ASSERT_FALSE(returned) << buffer_free_with_inner_buffers(&task_arguments);
		}
#endif
		ASSERT_FALSE(buffer_size(argument)) << buffer_free_with_inner_buffers(&task_arguments);
	}
