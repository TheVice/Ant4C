...
uint8_t Test_SHA3()
{
	...
	for (uint8_t i = 0; i < count; ++i)
	{
		...
		if (!buffer_resize(&output, 2 * UINT8_MAX))
		{
			buffer_release(&output);
			printf("%i %s\n", __LINE__, "FAILURE");
			return 0;
		...
		for (uint8_t j = 0; j < 4; ++j)
		{
			for (uint8_t k = 0; k < 2; ++k)
			{
				if (!buffer_resize(&output, 0) ||
					!SHA3_functions[4 + j](input + k, input + lengths[i] + k, &output))
				{
					buffer_release(&output);
					printf("i = %i j = %i %i %s\n", i, j, __LINE__, "FAILURE");
					return 0;
				}
				...
				if (!hash_algorithm_bytes_to_string(returned, returned + size, &output))
				{
					buffer_release(&output);
					printf("i = %i j = %i %i %s\n", i, j, __LINE__, "FAILURE");
					return 0;
				...
				returned += size;
				const uint8_t* expected = expected_outputs[4 * i + j];
				...
				if (k == (0 == memcmp(expected, returned, strlen(expected))))
				{
					if (k)
					{
						printf("%i\nhash '%s' should not be equal to expected at this point.\n%s\n", i, returned, "FAILURE");
