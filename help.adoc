...
If no file specific, all file with '.build' extension will be executed on Windows system. +
...
1) Run program. On Windows platform it search all file with '.build' extension and run it with default targets. If project do not specify such, global task will be executed. +
...
* Add to such one condition at unless __${\target::has-executed(''name of target'')}__. In a future probably just '${\target::has-executed(target::get-current-target())}'.
...
| Debug | "Debug" string will be added before message.
| Info | "Info" string will be added before message.
| Verbose | "Verbose" string will be added before message.
| Warning | "Warning" string will be added before message.
| Error | Will be used standard error output, "Error" string will be added before message.
| None | Mute echo message.
| NoLevel | No addition string before message will not be added.
|===
...
This is almost same if set 'failonerror' of failed task to the future. +
...
<project>
  <property name="folders"
    ...
    readonly="true" />
  <foreach item="String" in="${folders}" delim=",&#13;" trim="Both" property="folder">
    <echo unless="${string::empty(folder)}">'${folder}' -> '${environment::get-folder-path(folder)}'</echo>
  </foreach>
</project>
----
...
| blake2b-160 .4+^.^| Calculate hash with BLAKE2b algorithm with requested length.
| blake2b-256
| blake2b-384
| blake2b-512
|===
...
<project>
  ...
  <property
    name="algorithms"
    value="crc32, blake2b-160, blake2b-256, blake2b-384, blake2b-512"
    readonly="true" />
...
NOTE: At current release of Ant4C algorithms Keccak/SHA3 and BLAKE3 do not supported to calculate hash of files directly. However it can be done if first load content of file to the property and calculate hash sum of the property value.
...
|===
.s| Script function .s| Description
| <<hash-blake2b_160,blake2b-160>> .4+^.^| Calculate hash with BLAKE2b algorithm with requested length.
| <<hash-blake2b_256,blake2b-256>>
| <<hash-blake2b_384,blake2b-384>>
| <<hash-blake2b_512,blake2b-512>>
| <<hash-blake3,blake3/blake3-256>> ^| Calculate hash with BLAKE3 algorithm.
...
| <<hash-keccak_224,keccak-224>> .8+^.^| Calculate hash with Keccak algorithm with requested length for specific value. SHA3 use same algorithm, but with different values at internal padding stage.
| <<hash-keccak_256,keccak-256>>
| <<hash-keccak_384,keccak-384>>
| <<hash-keccak_512,keccak-512>>
...
*byte array* 'hash::blake2b-160'(*string*)

Calculate 20 bytes hash with BLAKE2b algorithm.

[[hash-blake2b_256]]
==== blake2b-256

*byte array* 'hash::blake2b-256'(*string*)

Calculate 32 bytes hash with BLAKE2b algorithm.

[[hash-blake2b_384]]
==== blake2b-384

*byte array* 'hash::blake2b-384'(*string*)

Calculate 48 bytes hash with BLAKE2b algorithm.

[[hash-blake2b_512]]
==== blake2b-512

*byte array* 'hash::blake2b-512'(*string*)

Calculate 64 bytes hash with BLAKE2b algorithm.

[[hash-blake3]]
==== blake3/blake3-256

*byte array* 'hash::blake3-256'(*string*)
//*byte array* 'hash::blake3'(*string*, *byte*)

IMPORTANT: Function will be renamed into 'blake3'.
...
<project>
  ...
  <foreach item="String" in="${contents}" delim="," trim="Start" property="content">
    <echo if="${version::less(program::version(), 2020.06)}">'${content}' -> ${hash::bytes-to-string(hash::blake3-256(content))}</echo>
  </foreach>
</project>
----
...
*byte array* 'hash::keccak-224'(*string*)

Calculate 28 bytes hash with Keccak algorithm.

[[hash-keccak_256]]
==== keccak-256

*byte array* 'hash::keccak-256'(*string*)

Calculate 32 bytes hash with Keccak algorithm.

[[hash-keccak_384]]
==== keccak-384

*byte array* 'hash::keccak-384'(*string*)

Calculate 48 bytes hash with Keccak algorithm.

[[hash-keccak_512]]
==== keccak-512

*byte array* 'hash::keccak-512'(*string*)

Calculate 64 bytes hash with Keccak algorithm.

[[hash-sha3_224]]
==== sha3-224

*byte array* 'hash::sha3-224'(*string*)

Calculate 28 bytes hash with Keccak/SHA3 algorithm.

[[hash-sha3_256]]
==== sha3-256

*byte array* 'hash::sha3-256'(*string*)

Calculate 32 bytes hash with Keccak/SHA3 algorithm.

[[hash-sha3_384]]
==== sha3-384

*byte array* 'hash::sha3-384'(*string*)

Calculate 48 bytes hash with Keccak/SHA3 algorithm.

[[hash-sha3_512]]
==== sha3-512

*byte array* 'hash::sha3-512'(*string*)

Calculate 64 bytes hash with Keccak/SHA3 algorithm.
...
| <<path-combine,path::combine>> | Combine two part of path into the system specific path. Can be used as path normalizer if sub paths present in difference forms.
...
Addition samples of using can be found at the develop branch of the project, at the https://github.com/TheVice/Ant4C/blob/4cc005f99954fa3d615edaa5bf8574d6fe8244fc/tests.xml#L4097[tests.xml file], at the 'project_load_from_content' test.
